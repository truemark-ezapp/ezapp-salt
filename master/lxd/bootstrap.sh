...
#if [[ ${#@} -ne 4 ]]; then
#    echoerror "Usage: ${0} <hostname> <domainname> <saltmaster-name> <ipv6=(true|false)>"
#    exit 1
#fi
...
/bin/echo "169.254.1.1 salt" > /etc/hosts
/usr/bin/apt-get -qy -o 'DPkg::Options::=--force-confold' -o 'DPkg::Options::=--force-confdef' install wget
cd /root
/usr/bin/wget -O bootstrap_salt.sh https://bootstrap.saltstack.com
/bin/sh /root/bootstrap_salt.sh -j "{\"master\": \"salt\"}" stable
